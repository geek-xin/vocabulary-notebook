...
首页右下角的按钮会把**应用本体**（不含词汇本数据）下载成 `vocabulary-notebook.html`。用
`file://` 直接打开这份副本就能离线使用，但**本地副本的书架是空的**——词汇本存在原站点的
浏览器存储里，不会随文件走。

本地副本每次打开会静默请求线上 `index.html`，比对里面的 `APP_VERSION`；版本更高时在下载按钮
上方提示，可一键下载新版。检查失败（离线、超时、读不到版本号）一律静默，不打扰学习。
